...
> 历史实验功能：从 0.5.0 起，项目主线是飞书多维表格插件。Chrome 扩展不能单独读取飞书勾选数据，不再作为推荐安装方式；本文仅保留给已有测试环境参考。

## 适用范围

Chrome 版可以接收飞书多维表格插件读取到的当前勾选记录及关联明细，也可以导入 CSV、TSV 或 JSON。在本机完成模板选择、A4 预览和打印，不需要业务服务器。0.7.0 的 PDF/长图导出仅在飞书插件主线中提供，历史 Chrome 侧栏不承诺支持该能力。
...
拖动侧边面板与网页之间的分隔边缘即可改变宽度。面板较窄时模板栏会自动收起，拖宽到足够宽时会自动恢复；也可以用“显示模板”和“隐藏模板”手动切换。Chrome 允许用户在浏览器设置中统一把所有侧边面板改到左侧，本扩展会服从这项全局设置。
...
3. 在左侧选择系统测试模板，或先到“模板管理”导入旧版排版模板文件。
4. 检查 A4 预览。
5. 点击“打印”进入系统打印。
...
- 导入数据不上传到项目开发者的服务器。
- 打印由 Chrome 与操作系统完成；历史侧栏不包含 0.7.0 的 PDF/长图导出。
